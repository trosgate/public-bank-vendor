from general_settings.models import MailingGroup, Branch
...
def get_mailing_list_excluding_sender(branch, user):
    customer = Customer.objects.filter(branch=branch).exclude(pk=user)
    branch_list = [f'{name.get_full_name()}<{mail.email}>' for name, mail in zip(customer, customer)]  
